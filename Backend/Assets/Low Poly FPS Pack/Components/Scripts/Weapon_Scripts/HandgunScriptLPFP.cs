...
public class HandgunScriptLPFP : MonoBehaviour {
	...
	public bool gunTakeoutAble = true;
	...
	private void Update () {
		...
		if (Input.GetButton("Fire2") && !isReloading && !isRunning && !isInspecting && gunTakeoutAble && !hasBeenHolstered) 
		{
			if (ironSights == true) 
			{
	...
	*/
		...
		//Shooting 
		if (Input.GetMouseButtonDown (0) && !outOfAmmo && !isReloading && !isInspecting && !isRunning && gunTakeoutAble && !hasBeenHolstered) 
		{
			anim.Play ("Fire", 0, 0f);
			if (!silencer) 
			{
